...
const url =
  target === 'peer' || target === 'replica'
    ? process.env.SUPABASE_REPLICA_DB_URL
    : process.env.SUPABASE_DB_URL || process.env.DATABASE_URL;

if (!url) {
  console.error('URL ausente. Use SUPABASE_DB_URL (local) ou SUPABASE_REPLICA_DB_URL (peer).');
  process.exit(1);
}
...
const cnt = await c.query(`select count(*)::int as n from ctos where dataset_id is not null`);
