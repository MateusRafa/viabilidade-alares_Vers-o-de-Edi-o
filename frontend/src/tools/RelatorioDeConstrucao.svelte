<script>
  /**
   * Relatório de Construção — formulário do setor de Implantação.
   * Baseado em FormularioEngenharia; evoluir aqui sem afetar Projetos nem o formulário legado.
   */
  ...
  function solicitarVoltarParaDashboardImplantacao(event) {
    ...
    void executarVoltarParaDashboardImplantacao();
  }
  ...
  async function executarVoltarParaDashboardImplantacao() {
    if (isTransitionLoading) return;
  ...
  onMount(async () => {
    ...
    if (typeof onBackRequest === 'function') {
      onBackRequest(() => solicitarVoltarParaDashboardImplantacao());
    }
